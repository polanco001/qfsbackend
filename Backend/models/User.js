...
router.get('/me', auth, async (req, res) => {
  try {
    ...
    // ✅ passcodeHash is excluded above for security, but the frontend still
    // needs to know WHETHER a passcode exists. Fetch just that field
    // separately and attach a plain boolean to the response.
    const passcodeCheck = await User.findById(req.user.id).select('passcodeHash');
    const userObj = user.toObject();
    userObj.hasPasscode = !!(passcodeCheck && passcodeCheck.passcodeHash);
